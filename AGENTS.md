...
- Treat the spatial-filter editor as a visible central window into a larger padded FFT plane. Keep separate controls for blocking only the visible/editable frequency window (outside high frequencies remain open) and blocking the entire Fourier plane (the image plane must be exactly dark).
